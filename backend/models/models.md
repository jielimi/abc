...
它具有查找模型ID和加载模型对象的方法。通常不需要将Model对象加载到内存中。模型对象不包含属性，模型的属性由关联的建模元素保留。IModelDb.Models.getSubModel和IModelDb.Models.tryGetSubModel是通过建模元素的ID，Code或Guid查找模型的便捷方法。Element.model是包含元素的模型的ID。
